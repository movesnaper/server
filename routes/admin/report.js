...
router.get('/ostatki/:id', async ({ db, params, query }, res) => {
  ...
  const { docs: used } = await db.find({
    selector: { ...selector(params.id),
      ref: { $exists: true },
      date: { $lt: range.end.format(format) }
  },
    fields: ['ref'],
    limit
  })
  ...
  const { docs } = await db.find({
    selector: { ...selector(params.id), 
      date: { $lte: range.end.format(format) },
      ref: { $exists: false },
      number: { $exists: true },
  },
    fields: ['_id', 'date', 'number', 'obespechenie', 'klient', 'ssuda'],
    limit
  })
  ...
  const items = result.reduce((cur, v, index ) => {
    const obespechenie = (v.obespechenie || [])
      .map((obespechenie, i) => {
        return !i ? { ...obespechenie, ...v, index: index + 1 } : obespechenie
      })
