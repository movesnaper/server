<template>
  <div class="row">
    ...
    <div class="col">
      <div class="row" >
        <div class="col-4">silver</div>
        <div class="col">price</div>
      </div>
      <div v-for="({ proba, price, add }, i) in [...silver, { add: true }]" :key="i"
      :class="['form-group row m-0 mt-2', { add }]">
        <input type="text" class="col-4 form-control mr-2" :value="proba"
        @change="({ target }) => onChange('silver', { proba: target.value }, i)">
        <input type="text" class="col form-control" :value="price"
        @change="({ target }) => onChange('silver', { price: target.value }, i)">
      </div>
    </div>
